#!/usr/bin/env python3
"""
Script para verificar dados no banco financeiro.db da raiz
"""
import sqlite3
import os

# Conectar ao banco na raiz
...
if total > 0:
    ...
    cur.execute("SELECT DISTINCT vencimento FROM contas_receber WHERE cliente LIKE '%MINERVA%' LIMIT 5")
    datas = cur.fetchall()
    print(f"📅 Datas MINERVA (amostra): {[d[0] for d in datas]}")
