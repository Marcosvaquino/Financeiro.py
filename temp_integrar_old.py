...
def integrar_manifesto_direto(caminho_arquivo):
    """
    ...
    try:
        ...
        backup_dir = os.path.join(os.path.dirname(caminho_arquivo), '..', '..', 'backups')
        os.makedirs(backup_dir, exist_ok=True)
        
        nome_arquivo = os.path.basename(caminho_arquivo)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_filename = f"backup_{timestamp}_{nome_arquivo}"
        ...
        print(f'🚛 INTEGRANDO DIRETAMENTE: {nome_arquivo}')
    # Carregar workbook
