...
try:
    ...
    pass
try:
    from .custo_frota import bp as custo_frota_bp
...
try:
    ...
    print("✅ Blueprint 'user_management' registrado com sucesso")
except Exception as e:
    print(f"❌ Erro ao registrar blueprint user_management: {e}")
    pass

# Função para obter menus do usuário baseado em permissões
def get_user_menus(user_id):
    """Obtém menus disponíveis para o usuário baseado em suas permissões"""
    if not user_id:
        return {}
    ...
    if user and user['perfil'] == 'admin':
        ...
        cursor.execute("""
            SELECT m.nome, m.rota, m.icone, m.categoria, m.ordem
    ...
    return menus_por_categoria

# Disponibilizar menus para todos os templates baseado em permissões
@app.context_processor
def inject_user_menus():
    user_menus = {}
    if session.get('logged_in') and session.get('user_id'):
        user_menus = get_user_menus(session.get('user_id'))
